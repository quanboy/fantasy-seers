...
3. ~~**Full-universe player ingest** — backend job pulling Sleeper's player dump; upsert all active QB/RB/WR/TE/K + team DEFs, replacing the stale 300-player seed. **Must land before invites go out** (the front door falls back to consensus rankings; a stale 2025 list with no rookies kills adoption on day one). Rankings depth does the relevance filtering, not ingest-time curation.~~ Done 2026-08-14. The full active universe is retained; the Master Sheet starts from the top 300 by Sleeper rank.
4. ~~**Daily ADP capture** — scheduled job writing Sleeper ADP into `adp_snapshots (player_id, source, captured_at, value)`, starting immediately. Single-source is fine for now: this series is the raw material for 2027's σ and the stock-chart pages. ADP not captured now is gone forever.~~ Done 2026-08-14. The daily player sync now upserts one UTC-day Sleeper snapshot per ranked player without making a second API call.
5. ~~**Front door** — Master Sheet becomes `/`; props feed demoted to a secondary nav item, otherwise untouched.~~ Done 2026-08-14. `/master-sheet` redirects to `/` so existing bookmarks keep working.
6. ~~**Format, hardcoded** — one app-wide scoring format stamped onto every snapshot at lock time (`scoring_format`/`superflex` columns from the Phase 0 salvage). No format UI this year — a settings dropdown would be a silent foot-gun (wrong-format scoring in January).~~ Done 2026-08-14; production was confirmed as `HALF_PPR`, single-QB on 2026-09-14. Repository defaults remain provisional so an unconfigured environment cannot lock boards.
7. ~~**Snapshot locking** — `locked_at` on `board_snapshots`; writes refused after lock; scoring only ever reads locked snapshots.~~ Done 2026-08-14 and hardened 2026-09-14. The idempotent admin global lock materializes untouched boards, stamps the app-wide format, freezes them as `SEASON_START`, and freezes consensus plus Sleeper ADP system baselines in the same transaction.

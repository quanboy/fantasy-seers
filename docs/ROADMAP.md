...
6. ~~**Format, hardcoded** — one app-wide scoring format stamped onto every snapshot at lock time (`scoring_format`/`superflex` columns from the Phase 0 salvage). No format UI this year — a settings dropdown would be a silent foot-gun (wrong-format scoring in January).~~ Done 2026-08-14 with provisional `FULL_PPR`, single-QB. The centralized format remains explicitly unconfirmed and must be confirmed or changed before the lock.
7. ~~**Snapshot locking** — `locked_at` on `board_snapshots`; writes refused after lock; scoring only ever reads locked snapshots.~~ Done 2026-08-14. The idempotent admin global lock materializes untouched boards, stamps the app-wide format, and freezes them as `SEASON_START`; it refuses to run while the format is provisional.

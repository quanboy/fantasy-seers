    ...
    <div className="min-h-screen bg-gray-950">
      ...
      <nav className="bg-gray-900 border-b border-gray-800 px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          <span className="text-blue-400 font-semibold text-sm sm:text-base">⚡ {user?.pointBank?.toLocaleString()} pts</span>
          <span className="text-gray-400 text-sm hidden sm:block">{user?.username}</span>
          <button onClick={logout} className="text-gray-500 hover:text-gray-300 text-xs sm:text-sm">Sign out</button>
        </div>
      </nav>
      ...
      <main className="max-w-2xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        {!loading && (
          <>
            {/* Open Props */}
            <h2 className="text-lg font-semibold text-white mb-4">Open Props</h2>
            ...
            {props.filter(p => p.status === 'RESOLVED').length > 0 && (
              <>
                <h2 className="text-lg font-semibold text-gray-500 mb-4">Resolved Props</h2>
